...
- (NSRect) imageContainerFrame
{
	...
	//make the image container 45 pixels up
	container.origin.y += 45;
	container.size.height -= 45;
...
- (NSRect) titleFrame
{
	...
	titleFrame.origin.y = container.origin.y + 23;
...
- (NSRect) subtitleFrame
{
	...
	//move the subtitleFrame inside the 'photo' background image
	NSRect container = [self frame];
	subtitleFrame.origin.y = container.origin.y + 3;
	
	//make sure the subtitleFrame has a 7px margin with the left/right borders
	float margin = subtitleFrame.origin.x - (container.origin.x + 7);
	if(margin < 0)
